 ...
 * There are some changes for cell modifiable.
 *
...
public class SudokuBoard implements Cloneable {
    ...
    protected final List<Boolean> states;
    ...
    public SudokuBoard(SudokuBoard other) {
        ...
        Collections.copy(states, other.states);
    }

    public boolean isSet(int cell) {
        return board[cell] != 0;
    }

    public boolean isSet(int x, int y) {
        return isSet(pointToCell(x, y));
    }
    ...
    public int getCell(int x, int y) {
        return getCell(pointToCell(x, y));
    }

    public void clearCell(int cell) {
        board[cell] = 0;
        states.set(cell, true);
    }

    public void clearCell(int x, int y) {
        clearCell(pointToCell(x, y));
    }
    ...
    public int getBoxRow(int x, int y) {
        return y - y % getSmallWidth();
    }

    public int getBoxRow(int cell) {
        return getBoxRow(cellToPoint(cell));
    }

    public int getBoxRow(Point point) {
        return getBoxRow(point.x, point.y);
    }

    public int getBoxCol(int x, int y) {
        return x - x % getSmallHeight();
    }

    public int getBoxCol(int cell) {
        return getBoxCol(cellToPoint(cell));
    }

    public int getBoxCol(Point point) {
        return getBoxCol(point.x, point.y);
    }

    public Point getBoxCorner(int x, int y) {
        return new Point(getBoxCol(x, y), getBoxRow(x, y));
    }

    public Point getBoxCorner(int cell) {
        return getBoxCorner(cellToPoint(cell));
    }

    public Point getBoxCorner(Point point) {
        return getBoxCorner(point.x, point.y);
    }
    ...
    public int pointToCell(Point p) {
        return pointToCell(p.x, p.y);
    }

    public Point cellToPoint(int cell) {
        return new Point(getRow(cell), getCol(cell));
    }
    ...
    public Set<Integer> getPeers(int cell) {
        return getPeers(cellToPoint(cell));
    }

    public Set<Integer> getPeers(Point point) {
        return getPeers(point.x, point.y);
    }
    ...
    public List<List<Integer>> getUnits(int cell) {
        return getUnits(cellToPoint(cell));
    }

    public List<List<Integer>> getUnits(Point point) {
        return getUnits(point.x, point.y);
    }
    ...
    public CandidateSet getOptions(int cell) {
        return getOptions(cellToPoint(cell));
    }

    public CandidateSet getOptions(Point point) {
        return getOptions(point.x, point.y);
    }

    public boolean isModifiable(int cell) {
        return states.get(cell);
    }

    public void setModifiable(int cell, boolean modifiable) {
        states.set(cell, modifiable);
    }

    public boolean isModifiable(int x, int y) {
        return states.get(pointToCell(x, y));
    }

    public void setModifiable(int x, int y, boolean modifiable) {
        states.set(pointToCell(x, y), modifiable);
    }
    public List<Integer> getEmptyCells() {
        List<Integer> list = new ArrayList<>();
        for(int i = 0; i < getNumCells(); i++) {
            if(!isSet(i)) list.add(i);
        }
    ...
    public boolean isAcceptable(int cell, int value) {
        return isAcceptable(cellToPoint(cell), value);
    }

    public boolean isAcceptable(Point point, int value) {
        return isAcceptable(point.x, point.y, value);
    }
